...
@objc open class ACFloatingTextfield: UITextField {
    ...
    @IBInspectable open var floatingLabelIsHidden: Bool = false
    ...
    fileprivate var labelIcon : UILabel?
    ...
    @IBInspectable open var lineColor : UIColor = UIColor(red: 183/256.0, green: 184/256.0, blue: 185/256.0, alpha: 1.0)
    
    /// Change line color when Editing in textfield
    @IBInspectable open var selectedLineColor : UIColor = UIColor(red: 24/256.0, green: 56/256.0, blue: 128/256.0, alpha: 1.0)
    
    /// Change placeholder color.
    @IBInspectable open var placeHolderColor : UIColor = UIColor.lightGray
    
    /// Change placeholder color while editing.
    @IBInspectable open var selectedPlaceHolderColor : UIColor = UIColor(red: 200/256.0, green: 200/256.0, blue: 200/256.0, alpha: 1.0)
    ...
    //Change iconColor
    @IBInspectable open var iconColor : UIColor = UIColor.gray
    
    @IBInspectable open var iconTxt : String = ""
    //MARK:- Set Text
    ...
    var padding = UIEdgeInsets(top: 0, left: 5, bottom: 0, right: 45
    )
    override open func textRect(forBounds bounds: CGRect) -> CGRect {
       let  bound = CGRect( x:4, y:4, width:bounds.size.width, height:bounds.size.height)
        return UIEdgeInsetsInsetRect(bound, padding)
    }
...
fileprivate extension ACFloatingTextfield {
    ...
    func initialize() -> Void {
        ...
        let lblIcon = UILabel(frame: CGRect(x: 0, y: 0, width: 40, height: 40))
        rightView.addSubview(lblIcon)
        self.rightView = rightView
        lblIcon.text = iconTxt
        lblIcon.backgroundColor = .clear
        lblIcon.font = UIFont(name: "CAPP", size: 18)
    ...
    func addFloatingLabel(){
        ...
        if labelPlaceholder?.superview != nil {
            ...
            labelFrame.size.height = self.frame.height - 40
            labelPlaceholder?.frame = labelFrame
            labelPlaceholder?.textColor = CappColors.darkBlue
            return
        }
        ...
        labelPlaceholder?.frame = CGRect(x:50, y:0, width:self.frame.size.width, height:self.frame.height)
    ...
    func addErrorPlaceholderLabel() -> Void {
        ...
        labelErrorPlaceholder?.font = Fonts.bold(withSize: 16)
    ...
    func showErrorPlaceHolder() {
        ...
        if self.errorText != nil && self.errorText != "" {
            ...
            UIView.animate(withDuration: 0.3, delay: 0, options: .curveEaseInOut, animations: {
                ...
                self.labelPlaceholder?.removeFromSuperview()
    ...
    func floatPlaceHolder(selected:Bool) -> Void {
        ...
        if selected {
            ...
            labelIcon?.textColor = selectedLineColor
        ...
        labelFrame?.size.height = 19
        let lblFrame = CGRect(x: (labelPlaceholder?.frame.minX)!, y: (labelPlaceholder?.frame.minY)! - 53, width: (labelPlaceholder?.frame.width)!, height: 14.33)
        
        
        UIView.animate(withDuration: 0.2, animations: {
            self.labelPlaceholder?.frame = labelFrame!;
            self.labelPlaceholder?.font = UIFont(name: (self.font?.fontName)!, size: 16)
            self.labelPlaceholder?.textColor = CappColors.darkBlue
            self.bottomLineView?.frame  =  bottomLineFrame!;
        })
    ...
    func resignPlaceholder() -> Void {
        ...
        let labelFrame = CGRect(x:-55, y:0, width:self.frame.size.width, height:self.frame.size.height)
