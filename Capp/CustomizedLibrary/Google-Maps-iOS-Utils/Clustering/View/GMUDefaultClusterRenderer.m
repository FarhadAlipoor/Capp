...
static const float kGMUMaxClusterZoom = 14;
  ...
  if ([self shouldRenderAsCluster:cluster atZoom:zoom]) {
    ...
    for (id<GMUClusterItem> item in cluster.items) {
      ...
      GMSMarker *marker = [self markerWithPosition:item.position
                                              from:fromPosition
                                          userData:item
                                       clusterIcon:[UIImage imageNamed: [item isRestaurantOpen] ? ([item isRestaurantSelected] ? @"marker_restaurant_selected" : @"marker_restaurant") : ([item isRestaurantSelected] ?@"marker_restaurant_closed_selected" : @"marker_restaurant_closed")]
